...
- **Choropleth dinamis** — gradasi warna kontinu dihitung dari rasio nilai data
...
Bagi projek berbasis module, gunakan varian ESM — API identik:
...
├── src/
│   └── map-renderer.core.js  # SUMBER CANONICAL library (satu-satunya tempat edit)
├── scripts/
│   ├── build-dist.mjs      # Generator: core -> kedua varian di dist/
│   └── test-dist.mjs       # Smoke test regresi kedua varian
├── dist/
│   ├── map-renderer.js     # Varian IIFE (global <script>) — hasil generate
│   └── demographic-map.esm.js  # Varian ES Module — hasil generate
├── locales/
...
Alur kerja library: edit `src/map-renderer.core.js` → jalankan
`node scripts/build-dist.mjs` → kedua varian di `dist/` diperbarui.

Prinsip pemisahan: logika library (`src/`, `dist/`), layer data (`data/`, `database/`), plugin (`locales/`), contoh konsumen (`examples/`), dan dokumentasi (`docs/`) tidak saling bercampur.
